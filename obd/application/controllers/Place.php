...
class User extends CI_Controller {
	...
	public function add_user()
	{
		...
		$this->form_validation->set_rules('blood_id', 'Blood Id');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$idata['password']=$this->input->post('mobile');
			$idata['password']=$this->input->post('date_of_birth');
			$idata['password']=$this->input->post('blood_id');
			$idata['password']=$this->input->post('password');
			$idata['blood_group']=$this->input->post('weight');
			$idata['age']=$this->input->post('address');
			$idata['mobile']=$this->input->post('place_id');
		...
		public function edit_user($id)
	{
		...
		$this->form_validation->set_rules('password', 'Password', 'required|min_length[5]|max_length[10]');
		$this->form_validation->set_rules('c_password', 'Confirm', 'required|matches[password]');
		$this->form_validation->set_rules('mobile', 'Mobile', 'required|regex_match[/^[01-9]*$/]');
		$this->form_validation->set_rules('date_of_birth', 'Date of Birth');
		$this->form_validation->set_rules('blood_id', 'Blood Id',);
		$this->form_validation->set_rules('weight', 'Weight');
		$this->form_validation->set_rules('status', 'Status');
